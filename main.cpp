...
#define T_MIN 3
#define T_MAX 30
#define T_THRESHOLD 0.8
#define BASELINE_ORIENT 27
#define TRACK_LEN 12
#define MIN_TRACK_GROUP_OFF 50
#define MIN_TRACK_GROUP_ON 20
// Tripwire
//#define VOFFSET -80
//#define HOFFSET -240
#define VOFFSET -0
...
int trackvalue = 5;
float speedratio = trackvalue>=5 ? trackvalue-4 : 1.0/(6-trackvalue);
...
// Trackbar callback
CALLBACK void speedCallBack(int pos, void *data)
{
    speedratio = (pos>=5 ? pos-4 : 1.0/(6-pos));
}
...
int main(int argc, char *argv[])
{
    ...
    cv::createTrackbar("Speed", "Passenger Count", &trackvalue, 10, speedCallBack, NULL);
    ...
    //DigitRecognizer dr(1, 10, 20, 30);  dr.test();
...
int trackClassify(std::string file)
{
    ...
    std::list<std::vector<cv::Point2f> > finalTrajectories; // eligible trajectories: 1) 2)
    ...
    while (true) {
        ...
        if(trajectories.size()<80) { //70 160 80
            //std::cout << ">>>>>>ADD NEW POINTS<<<<<<<" << trajectories.size() << std::endl;
        ...
        for (; iTrack!=trajectories.end(); i++) {
            ...
            if ((!status[i] || err[i]>12.0) && szTrack<=3) {
                ///std::cout << "Lost: " << iTrack->size() << std::endl;
                iTrack = trajectories.erase(iTrack);
                continue;
            }

            // stationary points
            //float displace = cv::norm(prevPoints[i]-nextPoints[i]);
            //if ((status[i] && err[i]<12.0) && displace<1e-4) {
            isValidTrack(*iTrack, mean_x, mean_y, var_x, var_y, length);
            if ((szTrack>3) && (var_x<1.0f) && (var_y<1.0f)) {
                ///std::cout << "Stationary Track: " << iTrack->size() << std::endl;
                iTrack = trajectories.erase(iTrack);
                continue;
            }
            ...
            if (iTrack->size()>60) {
                ///std::cout << "Loooooooooooooooong Track: " << iTrack->size() << std::endl;
                iTrack = trajectories.erase(iTrack);
                continue;
            }
        ...
        for (; iTime!=startTimes.end(); iTime++) {
            // majority direction for trajectories at frame iTime
            ...
            for (i=0, iTrack=trajectories.begin(); iTrack!=trajectories.end(); i++) {
                ...
                if (iTrack->at(0).x == *iTime) { // only use trajectories long enough
...
PRIVATE bool isValidTrack(const std::vector<cv::Point2f>& track, float& mean_x, float& mean_y, float& var_x, float& var_y, float& length)
{
    ...
    int nValidSeg = 0;
    std::vector<float> hod(8, 0);  // histogram of segment directions
    for( int i = 2; i < size; i++ ) {
        float temp_x = track[i].x - track[i-1].x;
        float temp_y = track[i].y - track[i-1].y;
...
PRIVATE int onOrOffStartEnd(const std::vector<cv::Point2f>& track)
{
    ...
    if (orient>45+BASELINE_ORIENT && orient<170+BASELINE_ORIENT) {//150
        return 0;
